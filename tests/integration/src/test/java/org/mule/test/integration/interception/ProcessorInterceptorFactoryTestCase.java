...
public class ProcessorInterceptorFactoryTestCase extends AbstractIntegrationTestCase {
  ...
  @Test
  public void implicitOperationParameters() throws Exception {
    flowRunner("die").run();
    ...
    assertThat(killInterceptionParameter.getParameters().keySet(), contains("config-ref"));
    final Object configRef = killInterceptionParameter.getParameters().get("config-ref");
    assertThat(configRef, instanceOf(HeisenbergExtension.class));
    assertThat(((HeisenbergExtension) configRef).getConfigName(), is("heisenberg"));
  }
